  ...
  const handleSearch = async () => {
    try {
      const response = await axios.get(
        `https://epics-final-i5eq-git-main-ravi02rrs-projects.vercel.app/api/search?query=${searchQuery}`
      );
  ...
  const handleInputChange = async (value) => {
    try {
      if (value.trim() === "") {
        ...
        const response = await axios.get(
          `https://epics-final-i5eq-git-main-ravi02rrs-projects.vercel.app/api/search-suggestions?query=${value}`
        );
        const data = response.data;
        setSuggestions(data);
      }
